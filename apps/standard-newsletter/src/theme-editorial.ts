...
/**
 * Editorial theme — the same warm paper/terracotta palette Standard Reader
 * applies (`apps/standard-reader/src/components/reader/theme.ts`), copied here
 * so both apps read as one product.
 *
...
/**
 * Newsreader (serif/display), Atkinson Hyperlegible Next (sans/UI), Spline Sans
 * Mono (mono) — the reader's families, loaded from Google Fonts in
 * `__root.tsx`. The reader additionally ships Capsize-generated metric-matched
 * fallbacks; the newsletter has no font pipeline, so the stacks fall back to
 * plain system families instead.
 */
