 ...
 * `app.standard-newsletter.subscriberList` and `app.standard-reader.site`
 * collections (create + update + delete). No broad `transition:generic`, so the
 * consent screen shows precisely the permissions the app uses.
 */
...
const SCOPE = [
  "atproto",
  atprotoScope.repo({
    // Two collections, both written on the author's behalf and both narrow:
    // their newsletter's subscriber list, and their sites' presentation
    // (`app.standard-reader.site`, which the reader's ingester mirrors). Still
    // no `transition:generic`, so the consent screen names exactly what the app
    // touches.
    collection: [SUBSCRIBER_LIST_COLLECTION, SITE_COLLECTION],
