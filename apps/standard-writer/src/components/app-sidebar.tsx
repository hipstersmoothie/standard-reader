      ...
      <div {...stylex.props(styles.sidebarScroll)}>
        <Link to="/" {...stylex.props(styles.brandLink)}>
          <span {...stylex.props(styles.wordmark)}>
            Standard{" "}
            <span {...stylex.props(styles.wordmarkAccent)}>Writer</span>
          </span>
        </Link>
        ...
        <nav {...stylex.props(styles.nav)}>
          ...
          >
            ...
            <span {...stylex.props(styles.navLabel)}>Sites</span>
          </Link>
        </nav>
        ...
        <div {...stylex.props(styles.followList)}>
          {publications.length === 0 ? (
            // An author with no publications yet has nothing to manage, so the
            // empty slot is the invitation to connect one rather than a dead
            // note.
            <Link to="/connect" {...stylex.props(styles.addRow)}>
              <Plus size={16} />
              <span {...stylex.props(styles.addLabel)}>Add a publication</span>
            </Link>
          ) : (
            publications.map((p) => (
              <Link
