...
const NO_DB =
  "[standard-writer] DATABASE_URL is not configured — the app reads all of its data from the shared Standard database.";
...
export const getShowcasePublications = createServerFn({
  ...
  try {
    ...
    console.error("[standard-writer] showcase publications failed:", error);
    return [];
  }
});
