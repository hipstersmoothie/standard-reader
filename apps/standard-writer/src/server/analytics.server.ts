 ...
 *
 * Every publication the author owns, whether or not it is mailed as a
 * newsletter — `newsletterConnected` says which. Returns `null` when no DB is
 * configured; the caller surfaces that as an error.
 */
export async function loadPublicationsFromDb(
  ...
  const pubRows = await db
    .select({
      ...
      newsletterConnectedAt: newsletterPublications.connectedAt,
    })
    .from(publications)
    // A left join, not an inner one: Standard Writer is the home for every
    // publication you own, and mailing one is a feature you switch on rather
    // than the price of appearing at all. `newsletter_publications` still
    // decides what gets *sent* — it just no longer decides what gets listed.
    .leftJoin(
      newsletterPublications,
      eq(newsletterPublications.publicationUri, publications.uri),
    )
  ...
  return pubRows.map((p) => {
    ...
    return {
      ...
      newsletterConnected: p.newsletterConnectedAt != null,
