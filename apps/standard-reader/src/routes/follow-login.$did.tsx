...
export const Route = createFileRoute("/follow-login/$did")({
  server: {
  ...
  component: FollowLoginPage,
});

/**
 * The author counterpart to `/subscribe-login/$did/$rkey`: a stripped-down sign
 * in that names who you're about to follow. No Standard Reader chrome and no
 * saved handles — someone arriving from an author's own site has no reason to
 * be shown the rest of the app before they've agreed to anything.
 */
function FollowLoginPage() {
  const { t } = useLingui();
  ...
  const redirect = buildAuthRedirectPath(`/follow/${meta.did}`);
  const authorName =
    meta.displayName?.trim() || meta.handle?.trim() || t`this author`;
