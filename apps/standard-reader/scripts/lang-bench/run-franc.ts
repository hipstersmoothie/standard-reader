...
import {
  ...
  proseLength,
} from "../../src/server/lang/sample.ts";

const [input, out] = process.argv.slice(2);
if (!input || !out)
  throw new Error("usage: run-franc.ts <corpus.jsonl> <out.jsonl>");
...
for (const r of rows) {
  ...
  const d = detectDocumentLanguage({
    title: r.title,
    description: r.description,
    textContent: r.text_content,
  });
  lines.push(
    JSON.stringify({
      uri: r.uri,
      sample,
      prose: proseLength(sample),
      franc: d?.code ?? null,
      franc_conf: d?.confidence ?? null,
    }),
  );
}
