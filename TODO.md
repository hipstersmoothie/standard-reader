...
- [x] **Article discussion — Constellation expansion** — widen Bluesky link paths (`embed.media.external`, alternate facet shape); below-fold **Cited in** rail plus **Related reading** merged with `network.cosmik.connection` margin graph edges; `pnpm scan:discussion-sources` for ongoing `/links/all` discovery. (Skyreader shares evaluated and excluded from Discussion — full-article HTML reshare does not fit the comment-card model.)
