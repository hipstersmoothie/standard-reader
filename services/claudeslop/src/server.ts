...
function xrpcError(res: ServerResponse, status: number, error: string, message: string): void {
...
export function startServer(db: LabelerDb, keypair: Secp256k1Keypair): void {
  ...
  httpServer.listen(config.port, () => {
    console.log(`[server] listening on :${config.port} as ${config.labelerDid}`);
  });
}
...
function setupSubscribeLabels(
  ...
  setInterval(() => {
    for (const [ws, seq] of clients) {
      ...
      clients.set(ws, newer[newer.length - 1]!.seq);
    }
  }, 1_000).unref();
}
