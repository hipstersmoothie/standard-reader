...
describe("tokenizeJson", () => {
  it("colors keys, strings, numbers, and booleans", () => {
    ...
    expect(tokens.some((t) => t.type === "key" && t.text.includes('"ok"'))).toBe(
      true,
    );
    expect(tokens.some((t) => t.type === "bool" && t.text === "true")).toBe(
      true,
    );
...
describe("tokenizeCurl", () => {
  it("highlights query curl flags and url", () => {
    ...
    expect(tokens.some((t) => t.type === "url" && t.text.startsWith("https://")))
      .toBe(true);
  });
  ...
  it("highlights POST curl flags", () => {
    ...
    expect(tokens.some((t) => t.type === "flag" && t.text.includes("-X POST")))
      .toBe(true);
    expect(
      tokens.some(
        (t) =>
          t.type === "flag" &&
          t.text.includes("Content-Type: application/json"),
      ),
    ).toBe(true);
  });
});
